...
# @Last Modified time: 2018-09-18 10:31:50
...
def pre_processing(start,end):
    ...
    #     # np.save(os.path.join(output_dir,r'left',str(f)+'.npy'),length)
    #     # continue
    #     # exit()
    #     labels=[]
    ...
    length=len(right)
    for f in range(start,end):
        if os.path.isfile(os.path.join(output_dir,r'right',str(f)+'.npy')):
            print(os.path.join(output_dir,r'right',str(f)+'.npy'))
            continue
        # np.save(os.path.join(output_dir,r'right',str(f)+'.npy'),length)
        # continue
        #exit()
        labels=[]
        start_time=time.time()

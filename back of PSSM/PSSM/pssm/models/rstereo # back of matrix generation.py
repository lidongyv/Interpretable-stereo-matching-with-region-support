...
# @Last Modified time: 2018-08-31 15:03:41
# -*- coding: utf-8 -*-
# @Author: lidong
        ...
        for z in range(10):
          ...
          for i in range(150):
              ...
              for j in range(1,max_r+1):
                  with torch.no_grad():
                    ...
                    if num==zero:
                      ...
                      index=plane_mask.nonzero()
                      if index.shape[0]<=1:
                        continue                         
                      index=index.float().cuda(2).unsqueeze(2)
                      num=(index.shape[0]*one).unsqueeze(1)
                    else:
                      index_t=plane_mask.nonzero().float().cuda(2)
                      if index_t.shape[0]<=1:
                        continue 
                      index_t=index_t.float().cuda(2).unsqueeze(2)
                      if index_t.shape[0]>index.shape[0]:
                        index=torch.cat((torch.cat((index,torch.zeros(index_t.shape[0]-index.shape[0],index.shape[1],index.shape[2]).cuda(2)),0),index_t),2)
                      else:
                        index=torch.cat((index,torch.cat((index_t,torch.zeros(index.shape[0]-index_t.shape[0],index_t.shape[1],index_t.shape[2]).cuda(2)),0)),2)
                      num=torch.cat((num,(index_t.shape[0]*one).unsqueeze(1)),1)

                    if s_num==zero:
                      s_plane_mask=plane_mask*s_mask[...,x1:x2,y1:y2]
                      s_index=s_plane_mask.nonzero()
                      if s_index.shape[0]<=1:
                        continue                         
                      s_index=s_index.float().cuda(2).unsqueeze(2)
                      num=(s_index.shape[0]*one).unsqueeze(1)
                    else:
                      s_plane_mask=plane_mask*s_mask[...,x1:x2,y1:y2]
                      s_index_t=s_plane_mask.nonzero()
                      if s_index_t.shape[0]<=1:
                        continue 
                      s_index_t=s_index_t.float().cuda(2).unsqueeze(2)
                      if s_index_t.shape[0]>s_index.shape[0]:
                        s_index=torch.cat((torch.cat((s_index,torch.zeros(s_index_t.shape[0]-s_index.shape[0],s_index.shape[1],s_index.shape[2]).cuda(2)),0),s_index_t),2)
                      else:
                        s_index=torch.cat((s_index,torch.cat((s_index_t,torch.zeros(s_index.shape[0]-s_index_t.shape[0],s_index_t.shape[1],s_index_t.shape[2]).cuda(2)),0)),2)
                      num=torch.cat((num,(s_index_t.shape[0]*one).unsqueeze(1)),1)                      


                    if j==1:
                      plane_mask=torch.where(object_r==j,one,zero)
                      index=plane_mask.nonzero().float().cuda(2).unsqueeze(2)
                      if index.shape[0]<=1:
                        continue    
                      s_plane_mask=plane_mask*s_mask[...,x1:x2,y1:y2]
                      l_plane_mask=plane_mask*l_mask[...,x1:x2,y1:y2]                                            
                      s_index=s_plane_mask.nonzero().float().cuda(2).unsqueeze(2)
                      l_index=l_plane_mask.nonzero().float().cuda(2).unsqueeze(2)
                      s_num=(s_index.shape[0]*one).unsqueeze(1)
                      l_num=(l_index.shape[0]*one).unsqueeze(1)
                      num=(index.shape[0]*one).unsqueeze(1)
                    else:
                      plane_mask=torch.where(object_r==j,one,zero)
                      index_t=plane_mask.nonzero().float().cuda(2)
                      if index_t.shape[0]<=1:
                        continue
                      print(index_t.shape)
                      index_t=index_t.unsqueeze(2)
                      s_plane_mask=plane_mask*s_mask[...,x1:x2,y1:y2]
                      l_plane_mask=plane_mask*l_mask[...,x1:x2,y1:y2]
                      s_index_t=s_plane_mask.nonzero().float().cuda(2).unsqueeze(2)
                      l_index_t=l_plane_mask.nonzero().float().cuda(2).unsqueeze(2)
                      s_num=torch.cat((s_num,(s_index_t.shape[0]*one).unsqueeze(1)),1)
                      l_num=torch.cat((l_num,(l_index_t.shape[0]*one).unsqueeze(1)),1)
                      num=torch.cat((num,(index.shape[0]*one).unsqueeze(1)),1)
                      if index_t.shape[0]>index.shape[0]:
                        index=torch.cat((torch.cat((index,torch.zeros(index_t.shape[0]-index.shape[0],index.shape[1],index.shape[2]).cuda(2)),0),index_t),2)
                      else:
                        index=torch.cat((index,torch.cat((index_t,torch.zeros(index.shape[0]-index_t.shape[0],index_t.shape[1],index_t.shape[2]).cuda(2)),0)),2)
                      if s_index_t.shape[0]>s_index.shape[0]:
                        ...
                        s_index=torch.cat((s_index,torch.cat((s_index_t,torch.zeros(s_index.shape[0]-s_index_t.shape[0],s_index_t.shape[1],s_index_t.shape[2]).cuda(2)),0)),2)
                      if l_index_t.shape[0]>l_index.shape[0]:
                        l_index=torch.cat((torch.cat((l_index,torch.zeros(l_index_t.shape[0]-l_index.shape[0],l_index.shape[1],l_index.shape[2]).cuda(2)),0),l_index_t),2)
                      else:
                        l_index=torch.cat((l_index,torch.cat((l_index_t,torch.zeros(l_index.shape[0]-l_index_t.shape[0],l_index_t.shape[1],l_index_t.shape[2]).cuda(2)),0)),2)              
              ...
              #print(num)
                  ...
                  # s_weights=self.cluster(l_sf[...,x1:x2,y1:y2][...,xp1:xp2,yp1:yp2],s_index)
                  # s_cost=torch.sum(torch.sum(plane*s_weights,-2,keepdim=True),-3,keepdim=True)/torch.sum(s_weights)
                  # l_weights=self.cluster(l_lf[...,x1:x2,y1:y2][...,xp1:xp2,yp1:yp2],l_index)

...
# @Last Modified time: 2018-08-31 15:59:27
# -*- coding: utf-8 -*-
# @Author: lidong
...
class rstereo(nn.Module):
    ...
    def forward(self, l,r,P,pre1,pre2):
        ...
        for i in range(150):
            ...
            #start_time=time.time()
            num=l_num=s_num=zero.unsqueeze(1)
            #print(torch.cuda.memory_allocated(2)/1e+6,num.shape)
            #1.49
            ...
            #       #tensor.unfold()
                  ...
                  if l_num[0,0]==zero:
                    l_plane_mask=plane_mask*s_mask[...,x1:x2,y1:y2]
                    l_index=l_plane_mask.nonzero()
                    if l_index.shape[0]<=1:
                      continue                         
                    l_index=l_index.float().cuda(2).unsqueeze(2)
                    l_num=(l_index.shape[0]*one).unsqueeze(1)
                  else:
                    l_plane_mask=plane_mask*s_mask[...,x1:x2,y1:y2]
                    l_index_t=l_plane_mask.nonzero()
                    if l_index_t.shape[0]<=1:
                      continue 
                    l_index_t=l_index_t.float().cuda(2).unsqueeze(2)

...
# @Last Modified time: 2018-08-26 22:27:39
# -*- coding: utf-8 -*-
# @Author: lidong
              ...
              if x2>500:
                for m in range(x1,x2+1):
                  ...
                  for j in range(min_d,max_d):
                    if y1-j>0:
                      l_r_o_t=r_lf[...,m:m+21,y1-j:y2-j]
                      cost_l.append(torch.where(l_mask_o==1,cosine_s(l_l_o[m-x1:m-x1+21],l_r_o_t),zero))
                    else:
                        #print(r_lf.shape,r_sf.shape)               
                        l_r_o_t=torch.cat([r_lf[...,m:m+21,0:j-y1]*zero,r_lf[...,m:m+21,0:y2-j]],-1)
                        cost_l.append(torch.where(l_mask_o==1,cosine_s(l_l_o[m-x1:m-x1+21],l_r_o_t),zero))
                  m+=21

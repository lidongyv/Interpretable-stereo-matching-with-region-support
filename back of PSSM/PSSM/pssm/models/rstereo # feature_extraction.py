...
# @Last Modified time: 2018-08-25 22:58:34
# -*- coding: utf-8 -*-
# @Author: lidong
...
class rstereo(nn.Module):
    ...
    def forward(self, l,r,P,pre1,pre2):
        ...
        #5710
        # for i in range(32):
        #   cost_volume.append(cosine_s(l_sf,torch.cat([r_sf[...,i:],r_sf[...,:i]],-1)))
        """
        #promotion
        ...
        for i in range(torch.max(P3).type(torch.int32)+1):
            ...
            if min_d>192:
              min_d=zero.long()
            max_d=torch.min(torch.min(max_d,min_d.float()+one*192),y2.float()).long()
            if max_d<min_d:
              max_d=min_d.float()+one*192
              max_d=max_d.long()

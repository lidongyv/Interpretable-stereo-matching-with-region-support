...
# @Last Modified time: 2018-08-27 21:21:57
# -*- coding: utf-8 -*-
# @Author: lidong
...
class rstereo(nn.Module):
    ...
    def forward(self, l,r,P,pre1,pre2):
        ...
        for i in range(torch.max(P3).type(torch.int32)+1):
            ...
            print(s_l_o.shape,s_r_o_t.shape)
            cost_volume.append(torch.where(sy_match.reshape(1,s_l_o.shape[-2],d)>=0,cosine_s(s_l_o,s_r_o_t),zero))
            #ground and sky

...
# @Last Modified time: 2018-08-28 19:46:18
# -*- coding: utf-8 -*-
# @Author: lidong
    ...
    def forward(self, l,r,P,pre1,pre2):
        ...
        for i in range(150):
            ...
            with torch.no_grad():
              ...
              sx_match=sx_match.repeat(1,d)
              sy_match=sy_match.repeat(1,d)
              sy_match-=torch.arange(min_d,max_d).cuda(2).repeat(s_match.shape[0],1).transpose(1,0).contiguous().view_as(sy_match)
              lx_match=lx_match.repeat(1,d)
              ly_match=ly_match.repeat(1,d)
              ly_match-=torch.arange(min_d,max_d).cuda(2).repeat(l_match.shape[0],1).transpose(1,0).contiguous().view_as(ly_match)

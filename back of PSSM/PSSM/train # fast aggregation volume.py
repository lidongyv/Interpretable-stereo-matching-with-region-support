...
# @Last Modified time: 2018-09-04 19:31:13
import sys
import torch
import visdom
import argparse
...
def train(args):
    ...
    #for epoch in range(0, args.n_epoch):
        ...
        print('training!')
        model.train()
        for i, (left, right,disparity,P,matching,plane,s_plane,l_plane) in enumerate(trainloader):
            #with torch.no_grad():
            #print(left.shape)
            left = left.cuda(0)
            right = right.cuda(0)
            ...
            P = P.cuda(1)
            s_plane[0]=s_plane[0].cuda(2)
            s_plane[1]=s_plane[1].cuda(2)
            for k in range(len(matching)):
                matching[k]=matching[k].cuda(1)
            for k in range(len(plane)):
                plane[k]=plane[k].cuda(1)
            for k in range(len(s_plane)):
                s_plane[k]=s_plane[k].cuda(1)
            for k in range(len(l_plane)):
                l_plane[k]=l_plane[k].cuda(1)
            #plane = plane.cuda(1)
            #s_plane=s_plane.cuda(1)
            #l_plane=l_plane.cuda(1)
            optimizer.zero_grad()
            #print(P.shape)
            outputs = model(left,right,P=P,matching=matching,plane=plane,s_plane=s_plane,l_plane=l_plane)

...
def pre_matching(start,end):
    ...
    for i in range(int(start),int(end)):
        ...
        max_d=np.where(match==-1,192,np.min([np.max([a,b],0)+variance_d,np.ones_like(match)*192],0))
        min_d=np.where(l_box[:,3]-l_box[:,1]>640,0,min_d)
        max_d=np.where(l_box[:,3]-l_box[:,1]>640,l_box[:,2]-l_box[:,0],0)
